...
async def _seed(db_pool):
    ...
    await db_pool.execute(
        ...
        "('9004', 'abandoned task', ARRAY['@me'], '2026-05-01', false)"           # long overdue
    )
    yield
    await db_pool.execute(
        "DELETE FROM todoist_tasks WHERE id IN ('9001', '9002', '9003', '9004')"
    )
...
async def test_long_overdue_task_drops_out_of_the_signal(db_pool, _seed):
    """A task overdue past the grace window is abandoned, not active work.

    Without a lower bound the guard latches open forever: one stale overdue
    task naming a repo suppressed every investigation for that repo (prod,
    2026-07-25 onward).
    """
    rows = await todoist.due_today_or_overdue(db_pool, "2026-06-20")
    contents = {r["content"] for r in rows}
    assert "abandoned task" not in contents  # 50 days overdue
    assert "fix aegis parser" in contents  # 1 day overdue, still counts
