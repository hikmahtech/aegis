"""run_task_investigation defers to a busy human session.

The `started` flag is what makes these falsifiable: asserting only on the
returned status would still pass if the run had been launched anyway.
"""
...
async def test_runs_when_the_busy_session_is_aegis_own_run():
