...
# Sent in the startup packet, NOT `SET` from `init=`. asyncpg runs `RESET ALL`
# when a connection goes back to the pool, which wipes any session `SET` — so
# an `init=` version works on a connection's first use and silently stops
# applying on every reuse after that. `RESET ALL` resets TO the startup value,
# so settings sent here survive. Verified in prod: with the `SET` form the
# second acquire reported an empty value and the scan fell back to 40
# candidates; with this form all three acquires reported `relaxed_order` and
# 400 candidates.
#
# Why it matters: pgvector's HNSW scan stops after `hnsw.ef_search` candidates
# (default 40) NO MATTER what LIMIT the query asks for. `KnowledgeStore.search`
# is a two-stage "ANN candidates, then filter" query, so that cap lands
# upstream of every source_type/tags filter — a search filtered to
# source_type='intelligence' (0.05% of the prod corpus) returned ONE document.
# `iterative_scan` keeps scanning until the LIMIT is satisfied instead, and is
# FASTER (16ms vs 28ms measured) because it stops as soon as it has enough.
# `relaxed_order` rather than `strict_order`: the outer query already re-sorts
# by similarity, so we don't pay for ordering pgvector would only have to redo.
#
# A pre-0.8 pgvector doesn't know the GUC. Postgres accepts an unknown dotted
# name in the startup packet as a placeholder rather than refusing the
# connection, so this stays safe on older builds.
_SERVER_SETTINGS = {"hnsw.iterative_scan": "relaxed_order"}
