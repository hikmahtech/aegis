"""The notes tools (#514): the user's Obsidian vault, read and appended to.
...
Every write is append-only and lands in the agent's own folder (the vault
layout's `agent_dir`); the rules live in `services/notes.py`, which is the
only thing that touches the vault. The write is owned by the calling agent,
else by whichever agent holds the `research` capability — never a literal id.
"""
...
# The capability whose holder owns a write made outside any agent's turn.
_OWNER_TAG = "research"
...
async def _owner(pool: asyncpg.Pool | None, ctx: ToolContext) -> str | None:
    """The agent a write is made for: the calling agent, else the holder of
    the `research` capability. None when there is neither."""
    if ctx.agent_id:
        return ctx.agent_id
    if pool is None:
        return None
...
async def _dispatch_notes_write(
    ctx: ToolContext, op: str, payload: dict, agent_id: str
) -> str:
...
async def _exec_note_search(
    ...
    *,
    ...
    """Search the user's Obsidian vault — every indexed note, this agent's own included — and return the best-matching notes with their paths. Read one in full with note_read.
...
async def _exec_note_read(
    ...
    *,
    ...
    """Read one note from the user's Obsidian vault by its path inside the vault, as note_search returns it. Encrypted blocks are never shown.
...
async def _exec_note_write(
    ...
    *,
    ...
    """Add a section to one of this agent's own notes in the user's vault, creating the note if it does not exist. Append-only: nothing already in a note is ever changed. Only notes in this agent's own folder can be written; give the path inside that folder, e.g. topics/rag.

    Args:
        path: The note, inside this agent's folder (the folder and .md are added if left off).
    ...
    owner = await _owner(pool, ctx)
    if not owner:
        return (
            "error: no agent owns this write (none is calling and none holds the "
            "research capability). Nothing was written."
        )
    layout = await get_layout(pool)
    # An empty heading becomes today's date: the user's today (`user_timezone`),
    # not the container's UTC one, in the layout's `date_heading_format`.
    payload, problem = nw.normalise(
        "write",
        {"path": path, "text": text, "heading": heading, "title": title},
        await user_now(pool),
        layout,
    )
    if problem:
        return f"error: {problem}. Nothing was written."
    return await _dispatch_notes_write(ctx, "write", payload, owner)
...
async def _exec_note_link(
    ...
    *,
    ...
    """Link one of this agent's own notes to another note, a book, a paper or a URL by appending one line: a [[wikilink]] for a note or book title, a markdown link for a URL. Append-only, inside this agent's folder only.

    Args:
        path: The note to add the link to, inside this agent's folder.
    ...
    layout = await get_layout(pool)
    payload, problem = nw.normalise(
        "link", {"path": path, "target": target, "label": label}, layout=layout
    )
    if problem:
        return f"error: {problem}. Nothing was written."
    return await _dispatch_notes_write(ctx, "link", payload, owner)
