...
repo of its own gets the `scratch` checkout.
...
class AgentRunFlow:
    ...
    async def run(self, inp: AgentRunInput) -> dict:
        ...
        while True:
            ...
            elapsed = int((workflow.now() - started_at).total_seconds())
            try:
                ...
                # deadline below is what bounds this loop.
            ...
            if elapsed >= deadline_s:
                ...
                workflow.logger.warning(
                    "agent_run_timeout run_id=%s elapsed_s=%d", run_id, elapsed
                )
                return _result(
                    "timeout",
...
def _provision_hint(reason: str, repo: str, repo_base: str) -> str:
    ...
    remedy — and for `scratch` there is no upstream to clone from, which is why
    the empty commit is part of it (a detached worktree needs a HEAD)."""
