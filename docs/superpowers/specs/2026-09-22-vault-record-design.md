...
- The code default for `entry.tag` becomes `#aegis/{agent}`, because a default must name nobody. It is a nested tag: Obsidian matches `#aegis` against every tag under it, so one tag finds or hides everything AEGIS wrote, and the agent's name is still on the block. `validate` already accepts it (a `#`, no spaces). A migration writes a `vault_layout` row holding the old literals only where a `notes_index_state` row exists, which means only on a deployment that has already synced a vault. It follows migrations 048 and 049.

**Config.** The owner sets `entry.tag` to `#aegis/{agent}` on the admin Vault page (§18, question 3).
...
|---|---|---|---|---|---|
| 1 | The journal moves to the `gtd` holder | `notes.py`, the three `_OWNER_TAG` sites, the seed yaml, the tag format, one migration, the agents seed | Code and seed. The owner sets `entry.tag`, the grants, the keywords and the persona text | The next nightly block opens `- #aegis/sebas`. `git log -1 --format=%an` on the vault is the `gtd` holder. Sunday's rollup logs `daylog_rollup_gathered n=7` across the change | No |
...
- **The owner move:** a rollup over a week whose first days carry `#raphael` and last days `#aegis/sebas` reads seven entries. `{agent}` resolves inside a nested tag. The migration writes nothing on a fresh database.
