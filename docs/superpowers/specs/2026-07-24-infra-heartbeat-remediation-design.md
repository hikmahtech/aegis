...
3. **Emit on transitions only** (steady state emits nothing — no per-tick spam):
   ...
   Synthetic alerts start `AlertInvestigationFlow` as ABANDONED child workflows (same pattern as `SentryPollFlow`), workflow id keyed on `aegis-heartbeat-<alertname>-<node|service>` so Temporal id-dedup suppresses accidental duplicates. Existing flow-side dedup (24h audit-log fingerprint, signature → open `@pandora` task) applies unchanged — with one enabling change: `build_alert_signature`'s infra branch becomes source-independent (`infra-class:{cluster}:{alertname}`, accepting `aegis-heartbeat` alongside alertmanager/prometheus/grafana) so a heartbeat-detected and an Alertmanager-pushed NodeDown collapse onto ONE signature/task instead of duplicating. Old-prefix rows in `alert_dedup_index` age out; a one-time missed recurrence-match is accepted.
   Recovery transitions (node back Ready, stuck service converged) write the same `audit_log` resolved rows the webhook writes (`action=alert_received, details.resolved=true`, deterministic fingerprint `aegis-heartbeat:<alertname>:<subject>`), so `check_alert_resolved` and the whole self-resolve machinery work unchanged for heartbeat alerts.
4. **Dead-man ping** — fire-and-forget GET to a healthchecks.io URL from a settings key (`infra_heartbeat_ping_url`); skipped when unset. Covers "AEGIS's own node (<core-host>) went down with the cluster" — healthchecks nags out-of-band when pings stop.
...
- **Self-resolve cancels nagging:** in `AlertInvestigationFlow`, the infra gate is raced against a periodic `check_alert_resolved` recheck (loop: await child result with a 3-min timeout; on timeout re-check resolution; resolved → signal the `InteractionFlow` child closed via `submit_response({"value": "self_resolved"})` — cleaner than cancellation, the interaction row resolves normally — post a "self-resolved" note, exit).
...
`ClarifyFlow` already spawns `AlertInvestigationFlow` for content-route matches (`spawn_kind == "pandora_investigation"`, `flows/clarify.py:299`), and the route table (`content_routes`, admin-configured, `core/src/aegis/services/content_routes.py`) already matches inbox tasks by regex. So the bridge is a small generic extension, not a new detector: routes gain an optional `alert_overrides: {source?, alertname?, severity?}` field, merged into `_pandora_alert_payload`'s alert dict. An admin-added route like `{"match": "regex", "value": "(?i)(node|swarm|service).*(down|unreachable|stuck)", "gate": true, "alert_overrides": {"source": "todoist-infra", "alertname": "NodeDown", "severity": "critical"}}` sends a hand-captured "noon is down" task through the FULL infra pipeline (deterministic homelab-gitops resolution via the alertname, auto-remediation, Gate 2 with Run fix), anchored to that `todoist_task_id` so all notes land on the user's own task. Picked up within the normal 5-min clarify tick; the keyword list lives in admin config, not code. Follows the clarify-watermark rule: spawn counts as a terminal state (`interaction_spawned`).
