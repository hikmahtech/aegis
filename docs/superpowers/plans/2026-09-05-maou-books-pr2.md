...
- A worktree has no `.venv` of its own: use the main checkout's interpreter, `/home/arshad/Workspace/hikmah/aegis/.venv/bin/python` (its editable installs resolve to the MAIN checkout, so `PYTHONPATH=core/src:worker/src:comms/src` is mandatory or you test the wrong code). Every `.venv/bin/python` and `.venv/bin/ruff` in this plan means that absolute path.
