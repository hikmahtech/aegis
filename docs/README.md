...
| Document | Covers |
|----------|--------|
| [How it works](how-it-works.md) | **Operator's guide** — the mental model, what runs when, GTD, the agent task executor, interactions, the alert pipeline, and the failure modes worth recognising |
...
| [Production](production.md) | Fork-owned image build + deploy, migrations, config plane, features that stay inert until you act, Slack scopes, alert routing, the signed life-data webhook, comms/Slack debugging |
