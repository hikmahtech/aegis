...
7. **Gate 2** — Open PR(s) / Run fix / Mute 24h (mutes the problem) / Acknowledge / Discard via Slack, only when there is a decision (#500, `gate2_needs_decision`): a fix branch, an actionable verdict's proposed commands (#518), an escalating alert, or a restart that did not stick. Any other verdict goes to the task (with any proposed commands, not run), the timeline and a chat ping with no card. Escalating alerts race the card against the hub seeing the problem resolve.
7.9. **The verdict goes to the knowledge store** once the outcome is known, tagged with it (`opened_pr` / `pr_failed` / `run_fix` / `discarded` / `muted` / `acknowledged` / `expired` / `self_resolved` / `no_card`, #502; patch id `kg-verdict-after-decision`). It used to be stored before the card.
