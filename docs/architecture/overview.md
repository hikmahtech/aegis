...
**Alert governance** — `pending_prs`. `alert_mutes` was dropped in PR 4a (mutes are `problems.muted_until`); `alert_dedup_index` is unread since PR 3b and is dropped after `scripts/hub_backfill.py` has run.
