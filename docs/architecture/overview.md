...
- `GitHubAlertFlow` — webhook-driven PR notifier (`pandoras-actor`). On `pull_request` `opened`/`reopened`/`ready_for_review` for a repo tracked in `resources`, posts a Slack card via `HomelabActivities.notify_pr_event`. On `closed`, hands the PR to `HubActivities.follow_fix_pr`, which moves the problem of a fix PR an investigation opened to `verifying` (merged) or `waiting_human` (closed unmerged) (#502, patch id `follow-fix-pr-on-close`). No longer investigates issues (repurposed 2026-06-27).
- `AgentChatReplyFlow` — synthesizes a personality reply, on two paths. `ClarifyFlow` spawns it WITH a task id for a comment on an agent-labelled Todoist task, and the reply is mirrored back onto that task. The chat @mention path (`POST /api/chat/agent-reply/trigger`) spawns it **taskless**: a message to an agent is a conversation, so nothing is captured and the agent calls `capture_to_inbox` itself if the exchange leaves work behind. The route used to capture every message before the agent had read it, which filled one Slack channel's inbox with 29 tasks named after passing questions.
- `AgentTaskFlow` — one run per agent-assigned Todoist task; abandoned child of the scheduled `AgentTaskSweepFlow`. Resolves a verb from the task's `source_tag` (the `agent_task_verbs` setting over generic defaults) and executes it behind interaction cards, or hands the task to the agent it is assigned to — see [`how-it-works.md`](../how-it-works.md#5-the-agent-task-executor).
...
- `DayLogFlow` (Raphael) — three schedule rows off one class (`mode: daily | weekly | monthly`). With the vault configured it writes the day, the week and the month into the vault's journal notes, which are the record (`NotesSyncFlow` indexes them as `source_type='note'`). Without the vault, or when a vault write fails, it files a dated knowledge entry (`aegis://daylog/<date>`) instead, and the rollups read both.
- `NotesSyncFlow` (Raphael, hourly) — pulls the user's Obsidian vault and indexes what changed as `source_type='note'`, encrypted blocks stripped. It leaves out `raphael/questions/`, because `ResearchFlow` keeps those answers in the store already. `NotesWriteFlow` performs one chat write (`note_write` / `note_link`), and `NotesBackfillFlow` (weekly) files daylog rows into the journal. `services/notes.py` is the only writer: insert-only, and a write counts once pushed. Spec: [`2026-09-12-raphael-notes-design.md`](../superpowers/specs/2026-09-12-raphael-notes-design.md).
- `ResearchFlow` (Raphael, event-driven) — one research question: the knowledge store (notes first), a web search, papers for an academic question, the Calibre library, the best pages read in full, and one answer that cites its numbered sources. The answer is saved to the store and to `raphael/questions/` in the vault. Started by the `research_topic` tool and by a `#research` task.
- `CalibreSyncFlow` (Raphael, daily) — one metadata row per Calibre book (`source_type='book'`), never the text; skips unchanged books and refuses to empty the index on a short catalogue.
...
- `DeliveryWatchdogFlow` (Pandora's Actor, hourly) — catches interaction cards that were never delivered and checks comms `/api/health` inbound liveness; both land on the problem hub (`undelivered_cards` on `interactions`, `comms_inbound_down` on `polling`), so a sustained outage is one problem, projected as one Todoist task (the chat channel is the thing that's down) and resolved on the first healthy tick.
- `FlowHealthWatchdogFlow` (Pandora's Actor, every 30 min) — the watchdog over AEGIS's own flows (#226). Alerts when the most recent N runs of one `workflow_type` are all `failed` (N=2, recency-ordered — a later success breaks the chain, so a resolved incident never re-alerts), and when an active `activities` row has had no *successful* run in `3x` its own cadence (derived from `schedule_cron`). Deduped by the problem hub (`services/hub_watch.py::reconcile_findings`), so a flow wedged all day is one card; recovery sends a `[FLOW OK]` card and resolves the problem, so the same flow breaking again alerts again. Silence one flow by muting its problem.
...
- `BooksWriteFlow` (Maou, event-driven) — one books write from a chat tool, off the chat turn's budget (#388). `ledger_post` / `ledger_reclassify` / `ledger_add_rule` validate the call, start this flow with a workflow id derived from the same content hash that makes the write idempotent, and wait 20s: a retried turn re-attaches to the run already doing the write instead of starting a second one, and a write that outlasts the wait is reported to Maou's channel by the flow itself rather than being lost. It calls exactly one activity, `books_write`, which runs `services/ledger_write.py` — the same module the tool validated against, so the two processes cannot drift. Not scheduled, so no `activities.yaml` row; not feature-flagged, because an unregistered flow's tasks are never picked up at all, while the `books_write` activity it calls is money-flagged and reports its own absence.
...
**`unblock` refuses an agent-assigned task, and that is the common case.** `@waiting` is overloaded: on a human task it means "blocked on someone", but it is also `agent_task.PARK_LABEL`, stamped at the *end* of every agent pass precisely to drop the task out of `find_actionable_tasks`. Stripping it there unblocks nothing — it re-queues finished agent work and recreates the infinite cooldown loop parking exists to prevent. Measured on live data, **19 of 25 open `@waiting` tasks were agent-parked**, so the guard (`UNBLOCK_SKIP_LABELS`) fires more often than the action does. `complete` and `comment` are unaffected — neither touches the parking state. The label list is a deliberate literal rather than a lookup of `agents.mention_aliases`: it is a safety guard and must hold when the DB is unreachable, and a test asserts it still matches `agent_task.ADDRESSABLE_ASSIGNEES` so a rename fails CI instead of silently re-queueing work.
...
**And the limit of the whole idea: email is a partial signal.** Jira sends no notification for a transition *you* make, so `email_task_links` can only ever close the tickets somebody else resolved. Measured on the author's account: of 15 open `APP-` tasks, **7 had no Gmail message about them at all, ever**, and the other 8 had mail carrying no resolution. `JiraSyncFlow` (`flows/jira_sync.py`, every 30 min) covers the rest by asking the tracker instead of waiting to be told — it reads open Todoist tasks whose title starts with an issue key, resolves them all in **one** JQL call, and closes the ones Jira reports resolved, leaving the same audit note. Read and write are separate activities, so `dry_run: true` simply never calls the second one; run it once whenever the key pattern or the project changes.

`fields.resolution`, not `status`, is the test. Status names are per-workflow — this board calls done "Deployed" and has an in-flight state called "Committed" — so a status allow-list would either miss the first or close the second. A resolution is non-null exactly when Jira considers the issue closed, whatever the column is called. A key Jira does not return (moved project, typo) is left alone, never treated as resolved.

Configure `jira_base_url` / `jira_email` / `jira_api_token` on the admin **Integrations** page, then restart the worker (connector config applies at boot). Any of the three blank ⇒ the flow reports `not_configured` and issues no request, so the schedule ships active and inert.
...
Both 2026-08 gaps are now closed in code: a capped cache hit re-teaches the sender ([#262](https://github.com/hikmahtech/aegis/issues/262)) and a rule can carry its own tags ([#263](https://github.com/hikmahtech/aegis/issues/263)). What remains is that a wrong verdict the cap never fires on still needs an override. Useful checks:
...
3. **Resource resolution** — a repo that claims the alert by label (`resources.metadata.alert_labels`) wins and is investigated as code; otherwise infra alerts (the `infra_alert_routing` settings list) resolve to the configured infra repo and try the one-shot auto-restart first (once per problem per window, #501: a problem back inside it goes to the investigation and one card instead), and the rest go deterministic service-match then LLM over the `resources` table. See `docs/infrastructure.md` → "Which repo an alert is investigated in".
4. **Knowledge context** — `gather_alert_knowledge` prepends the alert's runbook — a `runbooks` table row first (any spelling of the name), else `runbooks/<AlertName>.md` if present and non-stub — then appends up to three past verdicts on similar alerts (`alert_investigation` documents): a fix the operator took first, a discarded one never (#502). See `docs/infrastructure.md` → "The runbook an investigation reads" and "What Pandora remembers from your decisions".
...
7. **Gate 2** — Open PR(s) / Run fix / Run checks / Mute 24h (mutes the problem) / Acknowledge / Discard via Slack, only when there is a decision (#500, `gate2_needs_decision`): a fix branch, an actionable verdict's fix commands (#518; read-only checks never count, and code decides which commands are read-only, #641), an escalating alert, or a restart that did not stick. Any other verdict goes to the task (with any proposed commands, not run), the timeline and a chat ping with no card. Escalating alerts race the card against the hub seeing the problem resolve.
7.9. **The verdict goes to the knowledge store** once the outcome is known, tagged with it (`opened_pr` / `pr_failed` / `run_fix` / `run_checks` / `discarded` / `muted` / `acknowledged` / `expired` / `self_resolved` / `no_card`, #502; patch id `kg-verdict-after-decision`). It used to be stored before the card.
8. Comms notification (Slack) + the full report as a Todoist comment + `record_investigation` on the problem.

Every transition the flow makes (`investigating` → `waiting_human` / `fixing` / `resolved`) is a `state_change` on the problem, so the timeline, the digest and the next session read one record. The one move it cannot make is back out of `resolved` (#484): the alert source owns whether a problem is live, so a verdict that lands after the alert cleared is recorded as an `investigation` event and the problem stays resolved until a real occurrence reopens it. An opened fix PR leaves the problem `fixing`; `hub_fix` follows it from there (#502): the GitHub webhook's merge moves it to `verifying` (closed unmerged: `waiting_human`), and `HubSweepFlow` resolves it once the alert stays clear for `fix_verify_hours` or reopens it when the alert comes back. See `docs/infrastructure.md` → "After Open PR".
...
- An `input` card has an **Answer** button that opens a Slack text box (`text_open` → modal `text_submit`, up to 3,000 characters; `options.label` and `options.placeholder` word it). The answer is stored as `response.value`, the same shape the admin page sends, so the learning loop records nothing from it. The save is tried once, within 2 seconds, before the modal closes: anything but a confirmed save keeps the modal open with the text still in it, so a failed save never loses what was typed. A resend that finds the card already resolved compares the stored value, so a first try that landed late reads as answered. A blank answer is refused in the modal. The card then says "Answered" and never quotes the text, and no log line carries it (only the interaction id and the length). A card answered elsewhere or expired says so, and is not resolved again.
...
**Alert governance** — `pending_prs`, the hand-off between `stage_pending_pr` and `create_github_pr` once someone picks Open PR (title, body, branch; `status` `opened` / `failed`), pruned after 30 days; it carries `problem_id`, so a staged fix knows which problem it is for. What follows an opened PR reads the problem's `github_pr` link and events, not this table (#502). Everything the pre-hub alert path stored is gone: `alert_mutes` (migration 033 — mutes are `problems.muted_until`), `alert_dedup_index` (037 — identity is `problems.correlation_key`), `settings.alert_digest_buffer` (035 — the digest is a query over `problem_events`) and `pending_prs.alert_fingerprint` (038). `runbooks` (migration 044) holds per-alert runbooks as data, keyed on the alert name folded to lowercase letters and digits; the worker reads it before the built-in files (`services/runbooks.py`, `/api/admin/runbooks`).

**Problem hub** — `problems` (one row per thing that is wrong, identified by `services/hub.py::correlation_key`, never by a Todoist task), `problem_events` (every occurrence, resolution, report and note, idempotent on `(source, external_id)`), `problem_links` (`todoist_task`, `github_pr` and `plan_step` refs, and the `problem` link a rolled-over or merged problem keeps to the other). Written only through `hub.ingest_event` and `POST /api/hub/events`. The Todoist task is a *projection* of the problem (`services/hub_project.py`, PR 3a): created through the idempotent capture with `external_id = problem-<id>` once the problem earns attention, then kept up to date with comments — occurrences collapsed to one "N more" per 30 min, a resolve closes the task (never one the user claimed with `@me`, and under a mute too — a mute silences occurrences and returns, not recovery), a recurrence reopens it — and a status block re-rendered whole between `<!-- aegis:problem -->` markers in the description. A problem already resolved at its first projection gets no task. The one thing read back from the task is its completion: `HubSweepFlow` resolves a live problem whose task somebody completed, or reopens the task when the completion is the hub's own close from before the problem came back (`hub_project.reconcile_completed_tasks`, #473). `problems.metadata` holds the projection watermark (`projected_event_id`, `pending_occurrences`, `block_hash`). `work_sessions` (migration 034, PR 5a — the old `task_sessions`, widened) is the registry of who is on which task: AEGIS's own coding session per task (`owner='aegis'`, one live row enforced by a partial unique index, carrying the `CLAUDE_CONFIG_DIR` account so a later `--resume` keeps the same login) and the operator's, written by the `report_progress` tool. The collision check before every turn is a lookup over it, not an LLM judgement. A `plan` event carrying two or more `steps` becomes Todoist subtasks linked as `plan_step` (PR 5b), ticked off with `report_progress(step_done=N)`. The daily digest is a query over `problem_events` rather than a settings buffer, and `CleanupFlow` closes problems resolved more than `problem_close_days` ago, which is what frees a correlation key (PR 6a). Migration 030 (PR 1) ships the tables dark; producers move onto them in PR 3b. `service_state` (migration 031, PR 2) is what is happening to a subject right now — `deploying` / `maintenance` suppress (an occurrence inside the window is stored, counted and never projected; the problem sits `suppressed` until `HubSweepFlow` promotes it once the window passes without a resolution), `degraded` is information, `ok` deletes the row; `subject = '*'` is a wildcard. Written by the Ansible deploy role over `POST /api/hub/service-state`, the `set_service_state` chat tool, and the heartbeat, which clears a `deploying` row once the service has converged. `problems.group_key` (migration 040) makes one problem stand for the same failure on many things: a **group** is keyed `{class}:{subject_kind}` with correlation key `{class}:{subject_kind}:*`, and `ingest_event` absorbs into it every later occurrence of that class whose own key has no problem — so the seventh stuck Postiz post joins the group rather than opening a seventh task. `HubSweepFlow` proposes one (three or more live ungrouped problems of a class → one `think()` call → fold through `merge_problems`, retire the swallowed tasks, rename the survivor, post a card); `services/hub_group.py` is the whole of it. Never across classes, never on class `manual` (those problems are hand-written tasks), never on the count alone.
...
- `runbooks/<AlertName>.md` — generic per-alert runbooks (baked into worker image); stubs containing `TODO: fill in` are treated as absent. A row in the `runbooks` table for the same alert wins, and is where setup-specific runbooks go
