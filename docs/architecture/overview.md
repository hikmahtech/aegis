...
- `MoneyProcessFlow` (Maou, child) — single-email money hygiene: `store_receipt_email` → `fetch_message_body` → `store_receipt_body` → `load_receipts` → `classify_and_extract` → `upsert_charges` (the body fetch is best-effort — a failure leaves the stored snippet as the extractor's input). Spawned by `GmailIngestFlow` on `financial`/`payments` tags and by the weekly `ReceiptIngestFlow` safety-net. `ParentClosePolicy.ABANDON`; idempotent on `message_id` at the `store_receipt_email` step.
