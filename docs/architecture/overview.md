...
4. **Knowledge context** — `gather_alert_knowledge` prepends the alert's runbook — a `runbooks` table row first (any spelling of the name), else `runbooks/<AlertName>.md` if present and non-stub — then appends prior-incident context from KS. See `docs/infrastructure.md` → "The runbook an investigation reads".
...
**Alert governance** — `pending_prs`, a PR staged by Gate 2 and awaiting the human's answer; it carries `problem_id`, so a staged fix knows which problem it is for. Everything the pre-hub alert path stored is gone: `alert_mutes` (migration 033 — mutes are `problems.muted_until`), `alert_dedup_index` (037 — identity is `problems.correlation_key`), `settings.alert_digest_buffer` (035 — the digest is a query over `problem_events`) and `pending_prs.alert_fingerprint` (038). `runbooks` (migration 044) holds per-alert runbooks as data, keyed on the alert name folded to lowercase letters and digits; the worker reads it before the built-in files (`services/runbooks.py`, `/api/admin/runbooks`).
